...
var (
	...
	rflag = flag.String("r", "", "dump section data")
)
...
func dump(name string) {
	...
	if f.OptionalHeader != nil {
		...
		for _, y := range sym {
			...
			fmt.Printf("%-80s %#08x %s\n", y.Name, f.ImageBase+y.NameRVA, a)
		}
		fmt.Println()
	}
...
func dumpSectionData(f *peutil.File, name string) {
	for _, s := range f.Sections {
		if s.Name == name {
	...
	fmt.Println(name, "section not found")
}
